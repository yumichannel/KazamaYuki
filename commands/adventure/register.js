...
const data = {
    ...
    run: async function (bot = new Bot({}), msg = new Discord.Message, params = []) {
        ...
        try {
            //Question 1: name
            botmsg = await msg.author.send("1. Type your name, the one Yuki will call you:");
            ...
            //Question 2: sex
            botmsg = await msg.author.send(`2. ${new_acc.getName()}, Are you Shounen(1), Shoujo(2) or Futanari(3)?`);
            ...
            //Question 3: Race
            botmsg = await msg.author.send("3. Type your race: " + races_name.join(" | "));
            answer = await botmsg.channel.awaitMessages(m => {
                if (m.author.id === msg.author.id) {
            ...
            if (new_acc.name && new_acc.sex && new_acc.race) {
                //Init exp_max
                new_acc.exp_max = 100;

                new_acc.last_atd = null;
                new_acc.regis_from = Date.now();

                //Save
                bot.members.set(new_acc.user_id, new_acc);
                let reg_result = await bot.execsql(`insert into member_info 
                values("${new_acc.user_id}","${new_acc.name}",${new_acc.sex},${new_acc.race},${new_acc.level},${new_acc.exp},${new_acc.balance},null,${new_acc.regis_from})`);
                if (reg_result && reg_result.affectedRows) {
                    botmsg.channel.send("Welcome new member, for more information, read the guideline here:...");
                    return msg.channel.send(`${msg.member} joined as YH's member under the name of *${new_acc.name}*`);
                }
            }
